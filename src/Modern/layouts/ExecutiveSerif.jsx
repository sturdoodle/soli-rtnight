...
export const ExecutiveSerif = ({ data, themeColor, atsMode }) => (
  <main className={`bg-white px-16 py-20 min-h-[1122px] w-full shadow-xl ${atsMode ? 'text-black' : 'text-[#1a1a1a] '}`}>
    <header className={`text-center space-y-4 ${atsMode ? 'mb-10 border-b-2 border-black pb-8' : 'mb-16'}`}>
      ...
      <div className={`${atsMode ? 'text-lg font-bold' : 'flex items-center justify-center gap-6 text-[11px]  font-bold uppercase tracking-[0.1em] text-zinc-400'}`}>
        {!atsMode && <div className="h-px w-20 bg-zinc-200"></div>}
        {data.jobTitle}
        {!atsMode && <div className="h-px w-20 bg-zinc-200"></div>}
      </div>
      <div className={`flex flex-wrap justify-center gap-6 ${atsMode ? 'text-sm mt-4' : 'text-xs  opacity-60'}`}>
    ...
    <div className={`grid ${atsMode ? 'grid-cols-1 gap-12' : 'grid-cols-12 gap-16'}`}>
      ...
      {data.experience && data.experience.length > 0 && (
        <section className={atsMode ? 'space-y-10' : 'space-y-12'} aria-label="Professional Experience">
          <SectionTitle className={atsMode ? '' : 'border-b border-zinc-100 pb-2 mb-8 '} themeColor={themeColor} atsMode={atsMode}>Experience</SectionTitle>
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="mt-16" aria-label="Personal Projects">
            <SectionTitle className={atsMode ? '' : 'border-b border-zinc-100 pb-2 mb-8 '} themeColor={themeColor} atsMode={atsMode}>Personal Projects</SectionTitle>
            <div className="space-y-12">
      ...
      <aside className={`${atsMode ? 'space-y-10 mt-10' : 'col-span-4 space-y-16'}`}>
        {data.education && data.education.length > 0 && (
          <section aria-label="Education">
            <SectionTitle className={atsMode ? '' : 'border-b border-zinc-100 pb-2 mb-8 '} themeColor={themeColor} atsMode={atsMode}>Education</SectionTitle>
        ...
        {data.certifications && data.certifications.length > 0 && (
          <section aria-label="Certifications">
            <SectionTitle className={atsMode ? '' : 'border-b border-zinc-100 pb-2 mb-8 '} themeColor={themeColor} atsMode={atsMode}>Certifications</SectionTitle>
        ...
        {data.skills && data.skills.length > 0 && (
          <section aria-label="Professional Skills">
            <SectionTitle className={atsMode ? '' : 'border-b border-zinc-100 pb-2 mb-8 '} themeColor={themeColor} atsMode={atsMode}>Skills</SectionTitle>
            <div className={`space-y-8 ${atsMode ? '' : ''}`}>
