...
export const ElegantIndigo = ({ data, themeColor, atsMode }) => (
  <main className={`bg-white px-12 py-16 min-h-[1122px] w-full ${atsMode ? 'text-black print:p-0' : 'text-indigo-950 font-sans print:p-0 print:shadow-none'}`}>
    <header className={`${atsMode ? 'mb-10 text-center border-b-2 border-black pb-8' : 'flex justify-between items-start gap-12 mb-16 border-b border-indigo-100 pb-12'}`}>
    ...
    <div className={`grid ${atsMode ? 'grid-cols-1 gap-12' : 'grid-cols-12 gap-16'}`}>
      <div className={atsMode ? '' : 'col-span-12'}>
        {data.summary && (
          <section className="mb-16" aria-label="About Me">
            <h2 className={`${atsMode ? 'text-lg font-bold mb-4' : 'text-xs font-black uppercase tracking-[0.4em] mb-6 opacity-30'}`}>About Me</h2>
            <div className={`leading-relaxed ${atsMode ? 'text-[15px] text-black' : 'text-2xl font-light text-indigo-900/80'}`}>
      ...
      <div className={atsMode ? 'space-y-10' : 'col-span-7 space-y-16'}>
        <section aria-label="Work Experience">
          <SectionTitle className={atsMode ? '' : 'text-xs font-black uppercase tracking-[0.4em] mb-10 opacity-30'} themeColor={themeColor} atsMode={atsMode}>Experience</SectionTitle>
        ...
        {data.projects && data.projects.length > 0 && (
          <section className="mt-16" aria-label="Personal Projects">
            <SectionTitle className={atsMode ? '' : 'text-xs font-black uppercase tracking-[0.4em] mb-10 opacity-30'} themeColor={themeColor} atsMode={atsMode}>Personal Projects</SectionTitle>
            <div className="space-y-12">
      ...
      <aside className={atsMode ? 'space-y-10 mt-10' : 'col-span-5 space-y-16'}>
        <section aria-label="Core Skills">
          <SectionTitle className={atsMode ? '' : 'text-xs font-black uppercase tracking-[0.4em] mb-10 opacity-30'} themeColor={themeColor} atsMode={atsMode}>Core Skills</SectionTitle>
        ...
        <section aria-label="Educational Background">
          <SectionTitle className={atsMode ? '' : 'text-xs font-black uppercase tracking-[0.4em] mb-10 opacity-30'} themeColor={themeColor} atsMode={atsMode}>Academic Background</SectionTitle>
        ...
        {data.certifications && data.certifications.length > 0 && (
          <section aria-label="Certifications">
            <SectionTitle className={atsMode ? '' : 'text-xs font-black uppercase tracking-[0.4em] mb-10 opacity-30'} themeColor={themeColor} atsMode={atsMode}>Certifications</SectionTitle>
