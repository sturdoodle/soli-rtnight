...
export const SectionTitle = ({ children, className = "", style = {}, atsMode }) => (
  <h2 
    className={`text-sm font-bold mb-4 print:mb-2 ${atsMode ? 'text-black border-b-2 border-black pb-1' : 'uppercase tracking-[0.2em] ' + className}`}
    style={atsMode ? {} : style}
  >
    {children}
  </h2>
...
export const ExperienceItem = ({ exp, themeColor, atsMode }) => (
  <div className="group">
    <div className={`flex justify-between items-baseline mb-1 print:break-after-avoid print:[page-break-after:avoid] ${atsMode ? 'border-b border-gray-100' : ''}`}>
      <h3 className="text-lg font-bold" style={atsMode ? { color: 'black' } : { color: themeColor }}>{exp.role}</h3>
...
export const SkillCategory = ({ skill, themeColor, atsMode }) => (
  <div className="mb-4 print:mb-2 last:mb-0">
    <h3 className={`text-[10px] font-bold uppercase tracking-wider mb-1.5 print:mb-0.5 print:break-after-avoid print:[page-break-after:avoid] ${atsMode ? 'text-black' : 'opacity-50 print:opacity-100'}`} style={atsMode ? {} : { color: themeColor }}>
      {skill.category}
    </h3>
...
export const ProjectItem = ({ proj, themeColor, atsMode }) => (
  <div className="mb-6 print:mb-3 last:mb-0">
    <div className="flex justify-between items-center mb-1 print:mb-0 print:break-after-avoid print:[page-break-after:avoid]">
      <h3 className={`font-bold flex items-center gap-2 ${atsMode ? 'text-black' : ''}`}>
    ...
    <p className="text-xs font-bold uppercase tracking-wider mb-2 print:break-after-avoid print:[page-break-after:avoid]" style={atsMode ? { color: 'black' } : { color: themeColor }}>{proj.tech}</p>
...
export const CertificationItem = ({ cert, themeColor, atsMode }) => (
  <div className="flex items-start gap-3 mb-4 print:mb-2 last:mb-0">
    {!atsMode && (
      <div className="mt-1 opacity-50" style={{ color: themeColor }}>
        <ShieldCheck size={16} className="print:opacity-100" />
      </div>
    )}
    <div>
      <h3 className={`text-sm font-bold ${atsMode ? 'text-black' : 'opacity-90 print:opacity-100'}`}>{cert.name}</h3>
