...
const V5Sidebar = ({
  ...
  return (
    <aside className={`hidden lg:flex ${isCollapsed ? 'w-20' : 'w-72'} border-r border-black/5 dark:border-white/5 bg-[var(--v5-bg)] flex-col pt-4 print:hidden transition-[width] duration-300 relative h-full max-h-screen sticky top-0 !overflow-visible`}>
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-10 w-6 h-6 bg-[var(--v5-card)] border border-black/10 dark:border-white/10 rounded-full flex items-center justify-center shadow-[0_4px_10px_rgba(0,0,0,0.1)] hover:scale-110 transition-all z-[100] hidden lg:flex text-slate-500 hover:text-[var(--v5-heading)]"
      ...
      <div className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden custom-scrollbar">
        ...
        <nav className="flex-1 space-y-1 px-4 transition-all">
          ...
          {[
            ...
            <a
              ...
              title={link.title}
              className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-4 px-4'} py-3 rounded-2xl transition-all duration-300 hover:bg-black/5 dark:hover:bg-white/5 group`}
            >
              <div className={`p-2 rounded-xl transition-all group-hover:scale-110 ${isCollapsed ? 'bg-transparent' : ''}`} style={{ color: activeColor, backgroundColor: isCollapsed ? 'transparent' : `${activeColor}15` }}>
                <link.icon size={18} />
              </div>
              {!isCollapsed && (
        ...
        <div className={`mt-auto p-4 mb-2 flex flex-col items-center justify-center text-center transition-all duration-300`}>
          {isCollapsed ? (
             <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg shrink-0 transition-all hover:scale-110"
               style={{ backgroundColor: activeColor, boxShadow: `0 8px 20px -4px ${activeColor}60` }}>
               <span className="text-lg font-black" style={{ fontFamily: 'Absans, sans-serif' }}>qp</span>
             </div>
          ) : (
            <>
